...
## Current Status (MVP slice live)

Implemented in code and playable now:
- Guidance v1: click-to-guide using world coordinates. The scene moves Jane or the MagnetoSpeeder toward the selected point and draws a temporary cyan path highlight.
- Shield Window: imminent critical threats (< ~1.5s TTI) trigger a brief slow-mo bubble (timeScale reduction) if trust meets an emergency threshold; 6s cooldown prevents spam.
- Threat TTI halos: severity-colored markers with a time-to-impact ring per threat, tracked by unique id for consistent updates/resolution.

Interim success signals to watch while this slice is in playtests:
- Players use click-to-guide for medium/long moves (reduced WASD micro-corrections).
- Shield Window occurrences are rare but impactful, improving survivability around surprise threats.
- Players report “situational awareness” from halos without feeling cluttered.

## Next Steps (targeted polish for MVP)

High-confidence, low-risk additions for the next sprint:
- Arrival detection + trust loop: detect arrival/abandon; emit GUIDANCE_FOLLOWED or GUIDANCE_IGNORED; adjust Trust and surface feedback in UI.
- Guidance pathing: upgrade from straight-line to tile-aware navigation or simple waypoint avoidance; render dashed/breadcrumb path.
- Shield Window chip: on-screen indicator for cooldown state and trust gate; subtle audio cue; expose constants for tuning.
- Hover intel tooltip: nearest threat/opportunity summary in main panel; compact minimalist presentation.
- Analytics + A/B hooks: feature flags, time-in-mode, actions/min, shield usage, guidance adherence; enable small A/B on halo intensity.

Notes and constraints:
- Maintain NASA mission-control tone and high-intensity default; keep toggles available for minimal mode.
- Defer complex magic/bridges/barriers until MVP polish lands and metrics confirm ASI preference.
